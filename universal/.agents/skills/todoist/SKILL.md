...
: "${TODOIST_API_TOKEN:?TODOIST_API_TOKEN unset — get a token from Todoist → Settings → Integrations → Developer and add 'export TODOIST_API_TOKEN=<token>' to ~/.zshrc.local (never anywhere that gets committed)}"
