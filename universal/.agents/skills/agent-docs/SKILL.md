...
- Filename: `<KEY-or-PR>-<topic>-review.md` — e.g. `PROJ-1234-teardown-review.md`, `pr-4821-review.md`.
...
**follow it instead of these defaults** — such a skill may layer issue-key filenames or extra
verification banners on top of these conventions.
...
**Last verified:** <date> against <ref, e.g. origin/main (abc1234) / GitHub>
```
...
- **handoffs:** issue-key prefix when tied to one — `<KEY>-<topic>-handoff.md`
  (e.g. `PROJ-1234-audit-log-handoff.md`); otherwise descriptive `kebab-case-handoff.md`.
- **reviews:** `<KEY-or-PR>-<topic>-review.md` — e.g. `PROJ-1234-teardown-review.md`,
  `pr-4821-review.md`.
...
- **handoff** `merged` — verify the PR is merged (`git`/`gh`) **and** the tracking issue, if any, is
  closed (`gh`) — then move to `archives/handoffs/`. Never archive on a guess; if either is
...
1. **Handoffs** — for each, check the PR is merged (`git`/`gh` against the metadata `Branch`/`PR`)
   **and** the tracking issue, if any, is closed (`gh`). Both hold → `merged` → `archives/handoffs/`.
...
- `plan` → offer `/handoff` for each ready work item (and, with permission, tracking issues)
