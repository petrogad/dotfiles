---
name: plan
description: Decompose a research doc or a raw task into an actionable, user-guided plan and a set of work items (each ≈ one PR ≈ loosely one tracked issue), saved under ~/agents/plans/. Use when the user wants to turn findings or a goal into a concrete, sequenced plan of shippable units. May create tracking issues, but only after explicit permission.
...
for the `research → plan → handoff → pickup` suite. **Defer to any project-specific doc skill**
for the area being planned.
...
1. If given a research doc, Read it and carry its recommendation forward (link it, don't duplicate).
   If starting from a raw task, gather just enough context (code search, `gh` for existing issues)
   to decompose responsibly.
2. **Decompose into work items** where each item ≈ **one handoff ≈ one PR ≈ (loosely) one tracked
...
## Tracking issues — permission required

You **may** create tracking issues for the work items (e.g. GitHub issues via `gh`), but **only after
explicit user permission** — never silently. You may freely *pull* existing issue context. When the
user approves, create the issues and record their keys in the plan's roll-up table.
...
already done, offer to create tracking issues (with permission). Do not start implementation
automatically.
